...
const { createContent, getContentByUser, getContentStats } = require('../../../lib/db');
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (!url || !segmentType) {
        return res.status(400).json({ message: 'URL and segment type are required' });
      }
      ...
      return res.status(201).json({ id: result.id, message: 'Content saved successfully' });
    } catch (error) {
